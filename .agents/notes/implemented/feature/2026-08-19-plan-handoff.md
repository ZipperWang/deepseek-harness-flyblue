...
Package tests cover the previous plan-mode machine plus keep/compact/clear idle handoff, compact cancellation, and the no-factory clear fallback. `user-questions` and `ui-user-questions` pin `approve: string[]` and the four-action card. `SessionManager` selects a listed child after a live `plan/handoff`.
