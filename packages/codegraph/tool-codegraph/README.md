...
Registration does not require a `.codegraph/` index. A missing index or a failed engine load stays a successful tool result that tells the model to use `read`/`grep`/`glob`. The plugin never runs `codegraph init`.
...
- **Each workspace still needs a `.codegraph/` index** — the harness ships the engine and the tools, not a per-repo index. Creating the index remains `codegraph init` by the user.
