...
flowchart LR
  ...
  pkg_codegraph_index --> svc_codegraphIndex
...
| `ctx.planMode` | `core` | [`plan-handoff`](../packages/plan/plan-handoff) | - | - | - | 折叠已记录的计划／模式状态，在轮次边界刷新用户选择，渲染由部署方拥有的指导信息，注册 /plan，并在批准后选择保留、压缩或清空上下文再执行。 |
