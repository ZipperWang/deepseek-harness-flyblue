...
| `@deepseek-ai/dsh-plan-handoff` | `exit_plan_mode` | `ctx.tools`、`ctx.systemPrompt`、`ctx.userQuestions (execution time, opportunistic)` | `tool/call`、`plan/mode inactive on an approved review`、`tool/result` | - | 规划未激活时，exit_plan_mode 仍保留在面向模型的 schema 中，这样状态转换不会在规划策略变更之外额外造成工具目录变动。其执行路径会拒绝规划模式之外的调用；在规划模式下，它通过用户交互 seam 提交计划（批准／根据反馈继续规划），批准后会在步骤边界记录规划模式已停用。 |
...
| `@deepseek-ai/dsh-tool-codegraph` | `codegraph_explore` | `ctx.tools`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | codegraph_explore 是发行版附带 `@colbymchenry/codegraph` 引擎上默认列出的工具。缺少 `.codegraph/` 索引时仍返回成功结果。`extraTools` 可列出 status/node/search/callers/callees/impact/files。本目录按空的 extraTools 默认值采集。 |
...
| `@deepseek-ai/dsh-tool-ssh` | `ssh_exec`、`ssh_list` | `ctx.tools`、`ctx.ssh` | `tool/call`、`tool/result`、`ssh_exec` 的一次远程 SSH 命令 | - | 只有挂载 Host SSH 服务时才会出现这些可选工具。`ssh_exec` 每条命令只分发一次，并将分发后的连接丢失标记为结果未知，避免调用方自动重放非幂等命令。 |
...
来源：[`packages/plan/plan-handoff/src/index.ts`](../packages/plan/plan-handoff/src/index.ts)
...
codegraph_explore 是发行版附带 `@colbymchenry/codegraph` 引擎上默认列出的工具。缺少 `.codegraph/` 索引时仍返回成功结果。`extraTools` 可列出 status/node/search/callers/callees/impact/files。本目录按空的 extraTools 默认值采集。
...
在已配置的 SSH 主机上恰好执行一次命令。结果未知表示连接在分发后中断；不要自动重复非幂等命令。
...
```json
{
  ...
  "properties": {
    "host_id": {
      "type": "string",
      "description": "Configured SSH host id."
    },
    "command": {
      "type": "string",
...
来源：[`packages/ssh/tool-ssh/src/index.ts`](../packages/ssh/tool-ssh/src/index.ts)

### `ssh_list`

列出已配置的远程 SSH 主机，绝不返回密钥。
...
只有挂载 Host SSH 服务时才会出现这些可选工具。`ssh_exec` 每条命令只分发一次，并将分发后的连接丢失标记为结果未知，避免调用方自动重放非幂等命令。
